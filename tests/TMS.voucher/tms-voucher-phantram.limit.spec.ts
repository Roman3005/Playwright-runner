...
// Nhập thông tin
  ...
  await page.getByRole('radio', { name: 'Có giới hạn' }).first().check();
  await page.locator('input[name="max_discount_value"]').click();
  await page.locator('input[name="max_discount_value"]').fill('20.0000');
  await page.getByRole('radio', { name: 'Có giới hạn' }).nth(1).check();
  await page.locator('input[name="min_affect_value"]').click();
  await page.locator('input[name="min_affect_value"]').fill('10.0000');
  await page.getByRole('button', { name: 'loading Tiếp tục' }).click();
  await page.getByRole('button', { name: 'loading Tiếp tục' }).click();
