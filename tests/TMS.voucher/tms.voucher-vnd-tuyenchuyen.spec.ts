...
// Nhập thông tin
  ...
  await page.getByRole('radio', { name: 'Tuyến đường, Chuyến cụ thể M' }).check();
  await page.locator('.ant-select-selection-item').first().click();
  await page.getByText('Cao Bằng ---- Cà Mau').click();
  await page.locator('.ant-select-selection-overflow').click();
  await page.getByTitle('08:').locator('div').click();
  await page.getByText('TuyếnChuyếnCao Bằng ---- Cà').click();
  ...
  await page.getByRole('spinbutton', { name: 'Nhập giá trị' }).fill('2.5000');
  await page.getByRole('button', { name: 'loading Tiếp tục' }).click();
  await page.getByRole('button', { name: 'loading Tiếp tục' }).click();
